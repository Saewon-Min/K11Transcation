...
public class TicketDAO {
	...
	public void buyTicket(final TicketDTO dto) {
		...
		만약 하나라도 업무에 문제(예외)가 생긴다면 모든 업무는 rollback 처리된다.
		 이를 트랜잭션 처리라 한다.
		 */
		try {
			...
			template.update(new PreparedStatementCreator() {
				...
				public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
					...
					/*
					티켓 한장에 10000원 이라 가정하고, 구매 장수에 가격을 곱해서
					입력한다.
					 */
			...
			// 티켓 구매 장수에 대한 DB처리
			/*
				check제약조건에 의해 구매 장수가 5장을 초과하는 경우
			...
			/*
			티켓 구매장수가 5장 이하인 경우 정상처리되어 모든 작업이 commit된다.
			 */
			 ...
			 */
			System.out.println("제약조건을 위배하여 카드결제와 티켓구매 모두 취소 되었습니다.");
